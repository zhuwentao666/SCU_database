...
bool ExtendibleHash<K, V>::Find(const K &key, V &value) {
  std::lock_guard<std::mutex> lock(mutex_);//加锁
  //根据全局桶的深度取i个有效位，这样才能得到桶的编号
  size_t position = HashKey(key) & ((1 << depth) - 1);
  //使用map的find函数，返回的是被查找元素的位置，没有则返回map.end().
...
void ExtendibleHash<K, V>::Insert(const K &key, const V &value) {
  std::lock_guard<std::mutex> lock(mutex_);
    ...
    ++pair_count_;
    // 如果超出了设定的桶大小，需要分裂桶
    // 这时分裂桶是必需的，但是需不需要扩展桶数组取决于分裂前的桶的深度是小于还是等于全局深度
...
ExtendibleHash<K, V>::split(std::shared_ptr<Bucket> &b) {
    // 先创建一个新桶
    auto res = std::make_shared<Bucket>(0, b->depth);//创建与b同深度的桶
    // 注意：这里是while循环
    while(res->items.empty()) {
        ...
        for(auto it = b->items.begin(); it != b->items.end();) {
            if (HashKey(it->first) & (1 << (b->depth - 1))) {
                res->items.insert(*it);
                res->id = HashKey(it->first) & ((1 << b->depth) - 1);//此时新桶的id应该比原来多取一位,因为b的深度先加了
                it = b->items.erase(it);//从b中取走
            } else {
                ++it;
            }
        }
